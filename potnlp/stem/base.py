...
class PotawatomiLemmatizer():
    ...
    def lemmatize(self, tokens: List, token: str):
        ...
        if foundIdx:
            ...
            self._context["conjunct"] = self._check_condition(regex=r'^(ga|wa|éwi|égi|é|jé|gishpen)$', sentence=tokens, index=foundIdx[0], max_depth=0)
            self._context["negative"] = self._check_condition(regex=r'^(cho)$', sentence=tokens, index=foundIdx[0], max_depth=0)
            #ind_long_form_ctx = self._check_condition(regex=r'(?<!\w)(n|g|w)(gi|wi|de|da|dagi)(?!\w)', sentence=tokens, index=foundIdx[0], max_depth=0)
            self._context["prohibitive"] = self._check_condition(regex=r'^(gégo)$', sentence=tokens, index=foundIdx[0], max_depth=0)
            ## Short form will have to be figured out separately since there's some complexity involved
            
            return vii.stem(word, self._context)
